...
| 文件 | 用途 |
|------|------|
| `CURRENT_VERSION` | 记录 embed 版当前构建版本 |
| `CURRENT_VERSION_ASSETS` | 记录 assets 版当前构建版本 |

### 工作原理

```
定时任务 → 检查上游版本 → 对比版本文件 → 有更新则构建 → 更新版本文件
```
...
| 触发方式 | 是否检查版本文件 | 说明 |
|---------|-----------------|------|
| 定时 (`schedule`) | ✅ 是 | 仅在有新版本时构建 |
| Release | ❌ 否 | 强制构建 |
| 手动 (`workflow_dispatch`) | ❌ 否 | 强制构建，可指定任意版本 |

> 版本文件由构建工作流自动更新，无需手动维护。
